...
import com.stellarnaus.seabass.common.dto.model.SmsContentModel;
import com.stellarnaus.seabass.common.enums.ChannelType;
import com.stellarnaus.seabass.handler.domain.sms.MessageTypeSmsConfig;
...
import com.stellarnaus.seabass.handler.script.SmsScriptHolder;
...
public class SmsHandler extends BaseHandler implements Handler {
    ...
    public boolean handler(TaskInfo taskInfo) {
        ...
        try {
            ...
            for (MessageTypeSmsConfig messageTypeSmsConfig : messageTypeSmsConfigs) {
                // route之后返回具体的sms服务供应商TencentSmsScript，再调用它的send方法实现短信的最后发送
    ...
    private MessageTypeSmsConfig[] loadBalance(List<MessageTypeSmsConfig> messageTypeSmsConfigs) {
        ...
        MessageTypeSmsConfig supplierBackup = null;
        for (int i = 0; i < messageTypeSmsConfigs.size(); ++i) {
            if (index <= messageTypeSmsConfigs.get(i).getWeights()) {
                ...
                supplierBackup = messageTypeSmsConfigs.get(j);
                return new MessageTypeSmsConfig[]{supplier, supplierBackup};
            }
